...
exp_dist = pd.DataFrame(squareform(pdist(dfA.iloc[:, 1:3])), columns=dfA.marker.unique(), index=dfA.marker.unique())
exp_dist0 = pd.DataFrame(squareform(pdist(dfB.iloc[:, 1:3])), columns=dfB.marker.unique(), index=dfB.marker.unique())


# In[20]:


diff = exp_dist-exp_dist0
...
sns.heatmap(diff, mask=mask, cmap=cmap, center=0,
...
ax.set_title('Points change overview')
ax.set_xlabel('Point')
